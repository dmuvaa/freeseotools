...
export function LinkGraph({ nodes, edges, width = 800, height = 600 }: LinkGraphProps) {
    ...
    return (
        <div className="relative border rounded-xl overflow-hidden bg-surface-3 border-border shadow-md">
            <svg
                ref={svgRef}
                width={width}
                height={height}
            ...
            {selectedNode && (
                <div className="absolute top-4 right-4 bg-surface-1/90 backdrop-blur p-4 rounded-lg border border-border max-w-xs text-sm text-foreground shadow-xl">
                    <h4 className="font-bold text-foreground mb-1 truncate" title={selectedNode.id}>{selectedNode.id}</h4>
                    <p>Type: <span className="text-primary font-medium">{selectedNode.type}</span></p>
            ...
            <div className="absolute bottom-4 left-4 text-xs text-foreground-muted pointer-events-none">
                <div className="flex items-center gap-2">
                    <span className="w-3 h-3 rounded-full bg-error block"></span> Root Page
                </div>
                <div className="flex items-center gap-2 mt-1">
                    <span className="w-3 h-3 rounded-full bg-primary block"></span> Internal Link
                </div>
