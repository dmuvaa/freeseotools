...
async function getRedirectChain(url: string): Promise<{ chain: { url: string; status: number }[]; finalUrl: string; hops: number }> {
    ...
    for (let i = 0; i < 10; i++) {
        try {
            const r = await fetch(current, { method: "GET", redirect: "manual", headers: { "User-Agent": "Mozilla/5.0 (compatible; BlitzGeo/1.0)" }, signal: AbortSignal.timeout(5000) });
    ...
    try {
        ...
        try {
            res = await fetch(inputUrl, {
                headers: { "User-Agent": "Mozilla/5.0 (compatible; BlitzGeo/1.0; +https://blitzgeo.com)", "Accept": "text/html,application/xhtml+xml" },
                redirect: "follow",
